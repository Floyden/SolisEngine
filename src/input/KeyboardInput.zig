...
pub fn keyboardInputSystem(input: Global(Self), input_events: EventReader(KeyEvent)) void {
    ...
    while (input_events.next()) |event| {
        if (event.down) {
            if (!input.get().pressed.contains(event.scan_code)) {
                input.getMut().just_pressed.put(event.scan_code, null) catch @panic("OOM");
            }
            input.getMut().pressed.put(event.scan_code, null) catch @panic("OOM");
        } else {
            if (input.get().pressed.contains(event.scan_code)) {
                input.getMut().just_released.put(event.scan_code, null) catch @panic("OOM");
            }
            _ = input.getMut().pressed.swapRemove(event.scan_code);
        }
    }
}
